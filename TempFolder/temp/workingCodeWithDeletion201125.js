...
//   const before = mergedFloats.slice(0, start);
//   const after = mergedFloats.slice(end);

//   const newMerged = new Float32Array(before.length + after.length);
//   newMerged.set(before, 0);
//   newMerged.set(after, before.length);

//   mergedFloats = newMerged;
//   mergedMeshGlobal.updateData(mergedFloats.buffer);

//   // Remove metadata entry
//   const removedSplats = meta.endIndex - meta.startIndex;
...
//       m.startIndex -= removedSplats;
//       m.endIndex -= removedSplats;
//     }
//   }

//   // Clean selection visuals
...
//   if (startFloat >= mergedFloats.length || endFloat > mergedFloats.length) {
//     console.error(
//       "deleteObject: invalid range",
//       startFloat,
//       endFloat,
//       mergedFloats.length
//     );
...
//   console.log(
//     `Deleted ${removedSplats} splats from ${meta.fileName}. New total floats: ${mergedFloats.length}`
//   );
// }
...
function scaleObject(target, scaleFactor) {
  if (!target || target.startIndex == null || target.endIndex == null) {
    console.warn("scaleObject: invalid target", target);
    return;
  }
  if (!mergedFloats || !mergedMeshGlobal) {
    console.warn("scaleObject: missing merged data/mesh");
    return;
  }
  ...
  for (let i = start; i < end; i += FLOATS_PER_SPLAT) {
    mergedFloats[i] = cx + (mergedFloats[i] - cx) * scaleFactor;
...
// function deleteObject(meta, scene) {
//   if (!meta || !mergedFloats || !mergedMeshGlobal) {
//     console.warn("deleteObject: invalid state");
//     return;
//   }
...
//   if (
//     startFloat < 0 ||
//     endFloat > mergedFloats.length ||
//     startFloat >= endFloat
//   ) {
//     console.error(
//       "deleteObject: bad range",
//       startFloat,
//       endFloat,
//       mergedFloats.length
//     );
...
//   // Remove metadata for deleted object
//   const removedSplats = meta.endIndex - meta.startIndex;
//   objectMetadataList = objectMetadataList.filter((m) => m.id !== meta.id);

//   // Fix indices for objects after the removed one
//   for (const m of objectMetadataList) {
//     if (m.startIndex > meta.startIndex) {
//       m.startIndex -= removedSplats;
//       m.endIndex -= removedSplats;
...
window.addEventListener("keydown", (event) => {
  ...
  switch (event.key.toLowerCase()) {
    ...
    case "z":
      scaleObject(selectedObject, 1.05);
      break; // scale up
    case "x":
      scaleObject(selectedObject, 0.95);
      break; // scale down
