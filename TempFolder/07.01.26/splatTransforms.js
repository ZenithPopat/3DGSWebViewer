...
function ensureInteractiveNode(meta) {
  const it = state.interactiveTransform;

  if (it.node && it.activeMetaId === meta.id) {
    return it.node;
  }

  // Dispose old node if switching objects
  if (it.node) {
    it.node.dispose();
  }

  const node = new BABYLON.TransformNode(`interactive-${meta.id}`, state.scene);

  // Parent merged mesh temporarily
  state.mergedMesh.parent = node;

  it.node = node;
  it.position = node.position;
  it.rotation = node.rotationQuaternion ?? BABYLON.Quaternion.Identity();
  it.scaling = node.scaling;
  it.activeMetaId = meta.id;
