...
function createSceneGraphUI(scene, mergedMesh) {
  ...
  objectMetadataList.forEach((meta, index) => {
    ...
    //   // setObjectVisibility(meta, meta.visible);
    // });
...
function generateMetadata(gsMesh, fileName, startIndex) {
  ...
  // console.log("bounding box right now:", gsMesh.getBoundingInfo().boundingBox);
  // setTimeout(() => {
  //   gsMesh.refreshBoundingInfo();
  //   console.log("after 1s delay:", gsMesh.getBoundingInfo().boundingBox);
  // }, 2000);
...
//   if (!mergedFloats || !meta) return;

//   const start = meta.startIndex * FLOATS_PER_SPLAT;
//   const end = meta.endIndex * FLOATS_PER_SPLAT;

//   // Apply translation to that object's portion of merged buffer
...
//   // Update mesh GPU buffer
//   if (mergedMeshGlobal) {
//     mergedMeshGlobal.updateVerticesData(
//       BABYLON.VertexBuffer.PositionKind,
//       mergedFloats
//     );
//   }

//   // Update metadata transform
//   if (!meta.transform) meta.transform = { x: 0, y: 0, z: 0 };
//   meta.transform.x += dx;
//   meta.transform.y += dy;
//   meta.transform.z += dz;
...
//   console.log(
//     `Moved ${meta.fileName} by (${dx.toFixed(3)}, ${dy.toFixed(
//       3
//     )}, ${dz.toFixed(3)})`
//   );
// }
...
function translateObject(meta, dx, dy, dz) {
  ...
  console.log("Moving splats from", start, "to", end, "by", dx, dy, dz);
...
function scaleObject(target, scaleFactor) {
  ...
  // --- Compute center from splat positions directly ---
  let cx = 0,
    cy = 0,
    cz = 0;
  ...
  // --- Scale positions around this computed center ---
  for (let i = start; i < end; i += FLOATS_PER_SPLAT) {
...
function rotateObject(target, axis, angleDeg) {
  ...
  // --- Compute local center of object ---
  let cx = 0,
    cy = 0,
    cz = 0;
  const splatCount = target.endIndex - target.startIndex;
  ...
  // --- Precompute rotation based on axis ---
    ...
    try {
      ...
      scene.onAfterRenderObservable.addOnce(() => {
        if (mergedMeshGlobal.refreshBoundingInfo) {
          ...
          console.log("Bounding info refreshed (post-render).");
        }
      });
    } catch (e) {
